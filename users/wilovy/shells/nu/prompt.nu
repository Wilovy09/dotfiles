...
def get_git_status [] {
    ...
    let renamed = (do --ignore-errors { git diff --name-status } | lines | where $it starts-with 'R' | length)
    let deleted = (do --ignore-errors { git diff --name-status } | lines | where $it starts-with 'D' | length)
